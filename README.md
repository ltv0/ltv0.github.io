...
The output will be generated into the `dist/` folder.

## GitHub Pages Deployment

This repository is configured to deploy automatically to GitHub Pages from the built `dist/` folder.
...
3. The workflow installs dependencies, runs `npm run build`, uploads the `dist/` output, and deploys it via `actions/deploy-pages@v5`.
...
- `vite.config.ts` uses `base: './'` so the built site works correctly from the `dist/` folder.
- The workflow expects `dist/` as the production output directory.
